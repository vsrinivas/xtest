...
static std::unordered_set<uint32_t> src_hashes_fnv;
static std::unordered_set<std::string> src_hashes;
static std::unordered_set<uint32_t> dstA_hashes_fnv;
static std::unordered_set<std::string> dstA_hashes;
static int dstB_fd;
static int nSeq;
...
static int copy(int dst, const char *path, const struct stat *sb, char *src_hash) {
	...
	mkpath(dstB_fd, dirname(dstf), 0777);
	free(dstf);
...
int cb1(const char *path, const struct stat *sb, int typeflag, struct FTW *ft) {
	int rc;
	...
	case FTW_F: {
		...
		std::string hash(s);
		if (dstA_hashes.find(hash) != dstA_hashes.end()) {
			uint32_t fnv = fnvpath(path);
			if (dstA_hashes_fnv.find(fnv) == dstA_hashes_fnv.end()) {
				// md5 collision?
				++nDisaster;
				return -1;
			}
			...
			rc = copy(dstB_fd, path, sb, /*src_hash=*/s);
			if (rc == -1) {
				return -1;
			}
		}
...
int prefill_src(const char *path, const struct stat *sb, int typeflag, struct FTW *ft) {
	int rc;
	...
	case FTW_F: {
		...
		char *s = md5sum(path);
		printf("prefill(src) ==> %s, %s\n", path, s);

		std::string hash(s);
		if (src_hashes.find(hash) != src_hashes.end()) {
		} else {
			src_hashes.insert(hash);
		}

		uint32_t fnv = fnvpath(path);
		src_hashes_fnv.insert(fnv);
...
int prefill_dstA(const char *path, const struct stat *sb, int typeflag, struct FTW *ft) {
	int rc;
	...
	case FTW_F: {
		...
		char *s = md5sum(path);
		printf("prefill(dstA) ==> %s, %s\n", path, s);

		std::string hash(s);
		if (dstA_hashes.find(hash) != dstA_hashes.end()) {
		} else {
			dstA_hashes.insert(hash);
		}

		uint32_t fnv = fnvpath(path);
		dstA_hashes_fnv.insert(fnv);
...
// cphash <src directory> <dst A> <dst B>
// dest A, dest B directory must exist.
// Copy from src->dstB, doesn't copy file if it exists in dstA
//    hash table already.
// skips 0 size files.
int main(int argc, char *argv[])
{
	...
	auto dir = opendir(argv[3]);
	dstB_fd = dirfd(dir);
	if (dstB_fd == -1)
		return -1;

#if 0  // Unneeded - we hash files live.
	i = nftw(argv[1], prefill_src, 64, FTW_PHYS);
	if (i) {
		printf("src %d err %d\n", i, errno);
	}
#endif
	i = nftw(argv[2], prefill_dstA, 64, FTW_PHYS);
	if (i) {
		printf("dstA %d err %d\n", i, errno);
	}

	i = nftw(argv[1], cb1, 64, FTW_PHYS);
	if (i) {
		printf("%d err %d\n", i, errno);
	}
