...
static int match_db(char *url) {
	...
	key.data = url;
	key.size = strlen(url); // makes up for \n
...
/* MATCH reuses stuff from the map */
	...
	xcmd = calloc(1, s);
		// 34 is quote.
	snprintf(xcmd, s, "%s info %c%s%c", cmd, 34, it->path, 34);
/*
	system(xcmd);
 */
...
out:
/* disable if we use deferred DB store */
//	free(url);
//	free(score);
...
void process_one_item(struct item *it, struct barrier *bar) {
	...
	s = strlen(cmd) + strlen(it->path) + 32;
	cmdbuf = calloc(1, s);
		// 34 is quote
	snprintf(cmdbuf, s, "%s ls %c%s%c", cmd, 34, canon(it->path), 34);
